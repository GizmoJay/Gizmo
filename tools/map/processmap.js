...
module.exports = function parse (json, options) {
 ...
 map.tilesets.push({
            ...
            _.each(tileset.tiles, function (tile) {
                ...
                if (tile.animation && mode === "info") { handleAnimation(id, tileset.firstgid, tile); } else {
...
const parseLayer = function (layer) {
    ...
    if (name === "blocking" && mode === "client") {
        ...
        for (let k = 0; k < tiles.length; k++) {
            ...
            if (mode === "client") {
                if (tGid > 0) {
                    if (map.data[k] === undefined) { map.data[k] = tGid; } else if (map.data[k] instanceof Array) { map.data[k].unshift(tGid); } else { map.data[k] = [tGid, map.data[k]]; }
                }
            }
