...
define([
  ...
  "../entity/objects/item"
], (Camera, Tile, Player, Character, Item) => {
  return class {
    constructor(
      background,
      entities,
    ...
    loadSizes() {
      ...
      if (this.webGL) {
        this.map.loadWebGL(this.backContext);
      }
    }
    ...
    drawAnimatedTiles() {
      if (!this.animateTiles || this.webGL) {
        return;
      }
    ...
    drawSelectedCell() {
      if (!this.input.selectedCellVisible || this.input.keyMovement) {
        return;
      }
    ...
    saveFrame() {
      if (this.mobile && this.camera.centered) {
        return;
      }
    ...
    addLight(x, y, distance, diffuse, color, relative, objects) {
      const light = new Lamp(this.getLightData(x, y, distance, diffuse, color));
