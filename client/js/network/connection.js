...
class Connection {
  ...
  load() {
    ...
    this.messages.onSync(data => {
      ...
      if (data.armour) {
        entity.setSprite(this.game.getSprite(data.armour));
      }
    ...
    this.messages.onQuest((opcode, info) => {
      switch (opcode) {
        case Packets.QuestOpcode.AchievementBatch:
          this.interface.getQuestPage().loadAchievements(info.achievements);
    ...
    this.messages.onCamera((opcode, info) => {
      ...
      if (!this.renderer.camera.centered) {
        return;
      }
