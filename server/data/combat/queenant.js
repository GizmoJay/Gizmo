...
class QueenAnt extends Combat {
    ...
    spawnMinions () {
        ...
        for (var i = 0; i < self.minionCount; i++) { self.minions.push(self.world.spawnMob(13, self.character.x, self.character.y)); }
    ...
    getRandomTarget () {
        ...
        if (self.isAttacked()) {
            var keys = Object.keys(self.attackers);
                var randomAttacker = self.attackers[keys[Utils.randomInt(0, keys.length)]];
