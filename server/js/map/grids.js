...
class Grids {
    ...
    addToEntityGrid (entity, x, y) {
        ...
        if (entity && x > 0 && y > 0 && x < self.map.width && x < self.map.height && self.entityGrid[y][x]) { self.entityGrid[y][x][entity.instance] = entity; }
    }

    removeFromEntityGrid (entity, x, y) {
        const self = this;

        if (entity && x > 0 && y > 0 && x < self.map.width && y < self.map.height && self.entityGrid[y][x] && entity.instance in self.entityGrid[y][x]) { delete self.entityGrid[y][x][entity.instance]; }
    }
    ...
    getSurroundingEntities (entity, radius, include) {
        ...
        for (let i = -radius; i < radius + 1; i++) {
            for (let j = -radius; j < radius + 1; j++) {
                ...
                if (_.size(pos) > 0) {
                    _.each(pos, (pEntity) => {
                        if (!include && pEntity.instance !== entity.instance) { entities.push(pEntity); }
                    });
                }
            }
        }
