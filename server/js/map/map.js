...
class Map {
    ...
    load () {
        ...
        self.readyInterval = setInterval(() => {
            if (!self.world.ready) {
 if (self.readyCallback) { self.readyCallback(); } else {
    ...
    isPositionObject (x, y) {
        const self = this;
            ...
            const tiles = ClientMap.data[index];
            let isObject = false;

        if (tiles instanceof Array) {
 for (const i in tiles) {
 if (self.isObject(tiles[i])) { isObject = true; } else { isObject = self.isObject(tiles); }
}
}
 ...
 if (tileIndex > self.tilesets[id].firstGID - 1 &&
                    tileIndex < self.tilesets[id].lastGID + 1) { return self.tilesets[id]; }
}
}
