...
class Quests {
    ...
    load () {
        ...
        _.each(QuestData, (quest) => {
            if (questCount === 0) { self.quests[quest.id] = new Introduction(self.player, quest); } else if (questCount === 1) { self.quests[quest.id] = new BulkySituation(self.player, quest); }
 ...
 if (!isNaN(parseInt(ids[id])) && self.quests[id]) { self.quests[id].load(stages[id]); }
}
    ...
    updateAchievements (ids, progress) {
        ...
        for (let id = 0; id < ids.length; id++) {
 if (!isNaN(parseInt(ids[id])) && self.achievements[id]) { self.achievements[id].setProgress(progress[id], true); }
}
    ...
    getAchievement (id) {
        ...
        if (!self.achievements || !self.achievements[id]) { return null; }
 ...
 if (self.achievements[id].isFinished()) { count++; }
    ...
    getAchievementByNPC (npc) {
        ...
        for (const id in self.achievements) {
 if (self.achievements.hasOwnProperty(id)) {
 if (self.achievements[id].data.npc === npc.id && !self.achievements[id].isFinished()) { return self.achievements[id]; }
}
}
    ...
    getAchievementByMob (mob) {
        ...
        for (const id in self.achievements) {
 if (self.achievements.hasOwnProperty(id)) {
 if (self.achievements[id].data.mob === mob.id) { return self.achievements[id]; }
}
}
    ...
    isAchievementMob (mob) {
        ...
        for (const id in self.achievements) {
 if (self.achievements.hasOwnProperty(id)) {
 if (self.achievements[id].data.mob === mob.id && !self.achievements[id].isFinished()) { return true; }
}
}
    ...
    isAchievementNPC (npc) {
        ...
        for (const id in self.achievements) {
 if (self.achievements.hasOwnProperty(id)) {
 if (self.achievements[id].data.npc === npc.id && !self.achievements[id].isFinished()) { return true; }
}
}
