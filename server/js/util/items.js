...
Items.hasPlugin = (id) => {
    if (id in Items.Plugins) { return true; }

    return false;
};

Items.getPlugin = (id) => {
    if (Items.hasPlugin(id)) { return Items.Plugins[id]; }
...
Items.stringToId = (name) => {
    if (name in Items.Data) { return Items.Data[name].id; } else { log.error("Item: " + name + " not found in the database."); }
...
Items.getLevelRequirement = (name) => {
    ...
    if (Items.isWeapon(name)) { level = Items.Data[name].attack; } else if (Items.isArmour(name)) { level = Items.Data[name].defense; } else if (Items.isPendant(name)) { level = Items.Data[name].pendantLevel; } else if (Items.isRing(name)) { level = Items.Data[name].ringLevel; } else if (Items.isBoots(name)) { level = Items.Data[name].bootsLevel; }
...
Items.isWeapon = (string) => {
    if (string in Items.Data) { return Items.Data[string].type === "weapon" || Items.Data[string].type === "weaponarcher"; }

    return false;
};

Items.isArmour = (string) => {
    if (string in Items.Data) { return Items.Data[string].type === "armor" || Items.Data[string].type === "armorarcher"; }
...
Items.getShardTier = (id) => {
    if (id === 253) { return 1; } else if (id === 254) { return 2; } else if (id === 255) { return 3; } else if (id === 256) { return 4; } else if (id === 257) { return 5; }
};
...
Items.getMovementSpeed = (string) => {
    if (string in Items.Data) { return Items.Data[string].movementSpeed; }
