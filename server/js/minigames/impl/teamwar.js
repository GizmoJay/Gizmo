...
class TeamWar extends Minigame {
    ...
    load () {
        ...
        self.updateInterval = setInterval(() => {
            ...
            if (new Date().getTime() - self.lastSync > self.syncThreshold) { self.synchronize(); }
    ...
    buildTeams () {
        ...
        if (random === 1) { self.redTeam = tmp.splice(0, half), self.blueTeam = tmp; } else { self.blueTeam = tmp.splice(0, half), self.redTeam = tmp; }
    }
