...
class World {
    ...
    spawnEntities () {
        ...
        _.each(self.map.staticEntities, (data) => {
            ...
            if (isMob) {
                ...
                if (Mobs.Properties[key].hiddenName) { mob.hiddenName = Mobs.Properties[key].hiddenName; }
            ...
            if (isNpc) { self.addNPC(new NPC(info.id, instance, position.x, position.y)); }
    ...
    dropItem (id, count, x, y, ability, abilityLevel) {
        const self = this;
            const item = self.createItem(id, Utils.generateInstance(), x, y, ability, abilityLevel);
    ...
    addEntity (entity, region) {
        ...
        if (entity.instance in self.entities) { log.info("Entity " + entity.instance + " already exists."); }

        self.entities[entity.instance] = entity;

        if (entity.type !== "projectile") { self.region.handle(entity, region); }

        if (entity.x > 0 && entity.y > 0) { self.getGrids().addToEntityGrid(entity, entity.x, entity.y); }
    ...
    cleanCombat (entity) {
        ...
        _.each(this.entities, (oEntity) => {
            if (oEntity instanceof Character && oEntity.combat.hasAttacker(entity)) { oEntity.combat.removeAttacker(entity); }
        });
    }
    ...
    removeChest (chest) {
        ...
        if (chest.static) { chest.respawn(); } else { delete self.chests[chest.instance]; }
    }
 ...
 if (self.players[id].username.toLowerCase() === username.toLowerCase()) { return true; }
}
}
    ...
    getPlayerByName (name) {
        ...
        for (const id in self.players) {
 if (self.players.hasOwnProperty(id)) {
 if (self.players[id].username.toLowerCase() === name.toLowerCase()) { return self.players[id]; }
}
}
