...
class Enchant {
    ...
    generateAbility (tier) {
        ...
        if (self.hasAbility(self.selectedItem)) {
            const abilityName = Object.keys(Modules.Enchantment)[self.selectedItem.ability];
        ...
        _.each(Modules.Enchantment, (id, index) => {
            if (id === self.selectedItem.ability) { self.player.notify(`Your item has been imbued with the ${index.toLowerCase()} ability.`); }
        });
    }
