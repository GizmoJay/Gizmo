...
class Player extends Character {
    ...
    loadRegions (regions) {
        ...
        if (regions.gameVersion === config.gver) { self.regionsLoaded = regions.regions.split(","); }
    }
    ...
    intro () {
        ...
        if (self.hitPoints.getHitPoints() < 0) { self.hitPoints.setHitPoints(self.getMaxHitPoints()); }
    ...
    verifyRights () {
        ...
        if (config.moderators.indexOf(self.username.toLowerCase()) > -1) { self.rights = 1; }

        if (config.administrators.indexOf(self.username.toLowerCase()) > -1 ||
            config.offlineMode) { self.rights = 2; }
    }
    ...
    equip (string, count, ability, abilityLevel) {
        const self = this;
            ...
            let type; let id; let power;

        if (!data || data === "null") { return; }

        log.debug(`Equipping item - ${[string, count, ability, abilityLevel]}`);

        if (Items.isArmour(string)) { type = Modules.Equipment.Armour; } else if (Items.isWeapon(string)) { type = Modules.Equipment.Weapon; } else if (Items.isPendant(string)) { type = Modules.Equipment.Pendant; } else if (Items.isRing(string)) { type = Modules.Equipment.Ring; } else if (Items.isBoots(string)) { type = Modules.Equipment.Boots; }
        ...
        switch (type) {
            case Modules.Equipment.Armour:
                ...
                if (self.hasArmour() && self.armour.id !== 114) { self.inventory.add(self.armour.getItem()); }
    ...
    updatePVP (pvp, permanent) {
        ...
        if (self.pvp && !pvp) { self.notify("You are no longer in a PvP zone!"); } else { self.notify("You have entered a PvP zone!"); }
    ...
    getMovementSpeed () {
        const self = this;
            const itemMovementSpeed = Items.getMovementSpeed(self.armour.name);
            const movementSpeed = itemMovementSpeed || self.defaultMovementSpeed;
    ...
    getHit (target) {
        ...
        if (!self.hasSpecialAttack() || !isSpecial) { return new Hit(Modules.Hits.Damage, defaultDamage); }
    ...
    sendMessage (playerName, message) {
        ...
        const otherPlayer = self.world.getPlayerByName(playerName);
            const oFormattedName = Utils.formatUsername(playerName); // Formated username of the other player.
            const formattedName = Utils.formatUsername(self.username); // Formatted username of current instance.
