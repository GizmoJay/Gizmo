...
class Handler {
    ...
    load () {
        ...
        self.player.onHit((attacker, damage) => {
            ...
            if (self.player.combat.isRetaliating()) { self.player.combat.begin(attacker); }
        });
        ...
        self.player.onKill((character) => {
            if (self.player.quests.isAchievementMob(character)) {
                ...
                if (achievement && achievement.isStarted()) { self.player.quests.getAchievementByMob(character).step(); }
            }
        });
        ...
        self.player.connection.onClose(() => {
            ...
            if (self.player.ready) {
                if (config.hubEnabled) { self.world.api.sendChat(Utils.formatUsername(self.player.username), "has logged out!"); }
            }
        ...
        self.player.onPoison((info) => {
            ...
            if (info) { self.player.notify("You have been poisoned."); } else { self.player.notify("The poison has worn off."); }
