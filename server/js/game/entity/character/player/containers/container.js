...
class Container {
    ...
    load (ids, counts, abilities, abilityLevels) {
        ...
        if (ids.length !== self.slots.length) { log.error("[" + self.type + "] Mismatch in container size."); }

        for (let i = 0; i < self.slots.length; i++) { self.slots[i].load(ids[i], counts[i], abilities[i], abilityLevels[i]); }
    }
    ...
    canHold (id, count) {
        ...
        if ((self.type === "Bank" || maxStackSize === -1) && self.contains(id)) { return true; }
        ...
        for (let i = 0; i < self.slots.length; i++) {
 if (self.slots[i].id === id) { remainingSpace += (maxStackSize - self.slots[i].count); }
}
    ...
    remove (index, id, count) {
        ...
        if (Items.isStackable(id)) {
            if (count >= slot.count) { slot.empty(); } else { slot.decrement(count); }
        } else { slot.empty(); }
