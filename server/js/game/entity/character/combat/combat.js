...
class Combat {
    constructor (character) {
        ...
        self.character.onDamage((target, hitInfo) => {
            if (self.isPlayer() && self.character.hasBreakableWeapon() && Formulas.getWeaponBreak(self.character, target)) { self.character.breakWeapon(); }
    ...
    parseAttack () {
        ...
        if (self.character.hasTarget() && self.inProximity()) {
            if (self.character.target && !self.character.target.isDead()) { self.attack(self.character.target); }

            if (self.queue.hasQueue()) { self.hit(self.character, self.character.target, self.queue.getHit()); }
    ...
    parseFollow () {
        ...
        if (self.isMob()) {
            ...
            if (self.isAttacked() || self.character.hasTarget()) { self.lastAction = self.getTime(); }
        ...
        if (self.isPlayer()) {
            ...
            if (!self.inProximity()) { self.follow(self.character, self.character.target); }
        }
    }
    ...
    attack (target) {
        ...
        if (self.isPlayer()) { hit = self.character.getHit(target); } else { hit = new Hit(Modules.Hits.Damage, Formulas.getDamage(self.character, target)); }
    ...
    getNewPosition () {
        ...
        if (random === 0) { position.x++; } else if (random === 1) { position.y--; } else if (random === 2) { position.x--; } else if (random === 3) { position.y++; }
    ...
    canHit () {
        const self = this;
            ...
            const diff = currentTime - self.lastHit;
