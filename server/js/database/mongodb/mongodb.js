...
class MongoDB {
    ...
    getDatabase (callback, type) {
        const self = this;
            ...
            if (config.mongoAuth) { url = `mongodb://${self.user}:${self.password}@${self.host}:${self.port}/${self.database}`; }
            ...
            dataCursor.toArray().then((playerData) => {
                equipmentCursor.toArray().then((equipmentData) => {
                    regionsCursor.toArray().then((regionData) => {
                        if (playerData.length === 0) { self.register(player); } else {
    ...
    verify (player, callback) {
        ...
        self.getDatabase((database) => {
            ...
            dataCursor.toArray().then((data) => {
                if (data.length === 0) { callback({ status: "error" }); } else {
                    const info = data[0];

                    bcrypt.compare(player.password, info.password, (error, result) => {
                        if (error) throw error;

                        if (result) { callback({ status: "success" }); } else { callback({ status: "error" }); }
                    });
                }
            });
        });
            ...
            emailCursor.toArray().then((emailArray) => {
                if (emailArray.length === 0) {
                    usernameCursor.toArray().then((usernameArray) => {
                        if (usernameArray.length === 0) { callback({ exists: false }); } else { callback({ exists: true, type: "user" }); }
                    });
